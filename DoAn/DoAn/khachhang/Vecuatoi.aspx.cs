...
namespace DoAn.khachhang
{
    public partial class Vecuatoi : System.Web.UI.Page
    {
        ...
        protected void hien()
        {
            ...
            dt.AcceptChanges();
            gr_vecuatoi.DataSource = dt;
            gr_vecuatoi.DataBind();
        }
        ...
        protected void chitiet_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                txtMavx.Text = dt.Rows[i]["mavexe"].ToString();
                txtTthai.Text = dt.Rows[i]["trangthai"].ToString();
        ...
        protected void view_tt_ct_ve()
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                txtGiokh.Text = dt.Rows[i]["giokh"].ToString();
                txtTinhdi.Text = dt.Rows[i]["tinhdi"].ToString();
                txtTgdv.Text = dt.Rows[i]["thoigiandatve"].ToString();
                txtGioden.Text = dt.Rows[i]["gioden"].ToString();
                txtDiemden.Text = dt.Rows[i]["diemden"].ToString();
            }
        }
